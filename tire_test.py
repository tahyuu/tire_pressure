...
class Example(QtGui.QWidget):
    ...
    def updateDisplay(self):
        ...
        self.lcdNumber.display(text)
        #self.updateTaskList(False)
    def inputBoxInit(self):
        self.line_edit= QtGui.QLineEdit(self)
        self.line_edit.setGeometry(QtCore.QRect(250,50,self.desktop_width-500, 80))
        self.line_edit.setStyleSheet(''' text-align : center;
                                          height : 30px;
                                          font : 50px  ''')
        self.msg_label=QtGui.QLabel(self)
        self.msg_label.setStyleSheet(''' text-align : center;
                                          height : 30px;
                                          font : 40px  ''')
        self.msg_label.setGeometry(QtCore.QRect(10,150,self.desktop_width-30, 80))
        #self.connect(self.line_edit,QtCore.SIGNAL('textChanged(QString)'), self.inputBoxChanged)
        self.connect(self.line_edit ,QtCore.SIGNAL('returnPressed()'), self.inputBoxReturn)

        text = 'Your slot ID is : '.rjust(30) + "You Product SN is : ".rjust(30)
        self.msg_label.setText(text)
    def inputBoxReturn(self):
        text = self.sender().text()
        if time.time()-self.lastInputTime>3:
            self.slotId=''
            self.productId=''
             
        self.lastInputTime=time.time()
        if str(text).find('s')>=0:
            self.slotId=text
        elif str(text).find('p')>=0:
            self.productId=text
        text = ((('Your slot ID is : %s' %str(self.slotId).ljust(10)))+ ("You Product SN is : %s" %str(self.productId).ljust(10)))
        #text= "you input station id is:" + text
        self.line_edit.clear()
        self.msg_label.setText(text)
        self.msg_label.adjustSize()
        if self.slotId and self.productId:
            print "starting .................................."
    ...
    def taskListInit(self):
        ...
        self.widget_task_list.setGeometry(QtCore.QRect(10,250,self.desktop_width-30, self.desktop_height-300))
    ...
    def bs4_paraser(self,html):
        ...
        for row in all_div:
			...
			inFlag=False
                        for do in dolist:
                            if do[0]==object_ and do[1]==t:
			        inFlag=True
                        print inFlag
                        if not inFag:    
                            dolist.append([object_,t])
			...
			inFlag=False
                        for do in dolist:
                            if do[0]==object_ and do[1]==msg:
			        inFlag=True
                        print inFlag
                        if not inFag:    
                            dolist.append([object_,msg])
